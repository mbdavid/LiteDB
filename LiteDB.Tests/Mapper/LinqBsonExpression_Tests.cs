...
public class LinqBsonExpression_Tests
{
    ...
    public void Linq_Enumerables()
    {
        ...
        TestExpr<User>(
            x => !x.Phones2.Contains(new Phone { Number = 1 }),
            "(Phones2 ANY = { Number: @p0 }) = false",
            1);
        ...
        TestExpr<User>(
            x => x.Phones.Select(p => p.Number.ToString()).Any(p => p.StartsWith("51")),
            "MAP(Phones => STRING(@.Number)) ANY LIKE (@p0 + '%')",
            "51");
    }
    ...
    public void Linq_Predicate()
    {
        ...
        TestPredicate<User>(
            x => x.Active && x.Active && x.Active,
            "(((($.Active)=true) AND (($.Active)=true)) AND (($.Active)=true))");
    ...
    public void Linq_Nullables()
    {
        ...
        TestPredicate<User>(
            x => x.Latitude != null && x.Latitude > 0,
            "((Latitude != @p0) AND (Latitude > @p1))",
            BsonValue.Null,
            0);
        TestPredicate<User>(
            x => x.Latitude.HasValue && x.Latitude > 0,
            "((((IS_NULL(Latitude) = false)) = true) AND (Latitude > @p0))",
            0);
    }
    ...
    public void Linq_Methods()
    {
        ...
        TestExpr<User>(
            x => x.CreatedOn.Equals(new DateTime(2019, 1, 1)),
            "CreatedOn = DATETIME(@p0, @p1, @p2)",
            2019,
            1,
            1);
        ...
        TestExpr<User>(
            x => Guid.Parse("1A3B944E-3632-467B-A53A-206305310BAC"),
            "GUID(@p0)",
            "1A3B944E-3632-467B-A53A-206305310BAC");
    ...
    public void Linq_New_Instance()
    {
        ...
        TestExpr(
            x => new Guid("1A3B944E-3632-467B-A53A-206305310BAC"),
            "GUID(@p0)",
            "1A3B944E-3632-467B-A53A-206305310BAC");
        ...
        TestExpr<User>(
            x => new User { Name = x.Name.ToUpper(), Salary = x.Salary * 2 },
            "{ Name: UPPER($.Name), Salary: ($.Salary * @p0) }",
            2);
    }
    ...
    public void Linq_Composite_Key()
    {
        ...
        // using 2 levels
        TestExpr<User>(
            x => x.Address == new Address { Number = 1, City = new City { Country = "BR", CityName = "POA" } },
            "(Address = { Number: @p0, City: { Country: @p1, CityName: @p2 } })",
            1,
            "BR",
            "POA");
    }
    ...
    public void Linq_Coalesce()
    {
        ...
        TestExpr<City>(
            x => (x.CityName ?? x.Country) == DateTime.Now.Year.ToString(),
    ...
    public void Linq_Array_Contains()
    {
        ...
        TestExpr<User>(
            x => ids.Where(q => q == x.Id).Count() > 0,
            "(COUNT(FILTER(@p0 => (@=$._id))) > @p1)",
            new BsonArray { 1, 2, 3 },
            0);
    }
    ...
    public void Linq_InvocationExpression()
    {
        ...
        Expression<Func<User, bool>> exprMerged = Expression.Lambda<Func<User, bool>>(
            Expression.AndAlso(exprLeft.Body, invokedExprRight),
            exprLeft.Parameters);
    ...
    [DebuggerHidden]
    private BsonExpression TestExpr(
        Expression<Func<object, object>> expr,
        BsonExpression expect,
    ...
    [DebuggerHidden]
    private BsonExpression TestPredicate<T>(
