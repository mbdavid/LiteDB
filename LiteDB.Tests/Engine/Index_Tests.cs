...
public class Index_Tests
{
    ...
    public void Index_With_No_Name()
    {
        using (var db = new LiteDatabase("filename=:memory:"))
        {
            ...
            indexes.Invoking(indexCol => indexCol.FindOne("collection = 'users' AND name = 'namelast'"))
                .Should()
                .NotBeNull();
            indexes.Invoking(indexCol => indexCol.FindOne("collection = 'users' AND name = 'namefirst'"))
                .Should()
                .NotBeNull();
        }
    }
