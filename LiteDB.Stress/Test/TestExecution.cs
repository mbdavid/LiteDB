...
public class TestExecution
{
    ...
    private void ReportPrint(StringBuilder output)
    {
        ...
        output.AppendLine(
            $"Garbage Collector: gen0: {GC.CollectionCount(0)}, gen1: {GC.CollectionCount(1)}, gen2: {GC.CollectionCount(2)}");
    ...
    private void ReportSummary(StringBuilder output)
    {
        ...
        foreach (var task in _file.Tasks)
        {
            ...
            var count = _threads.Values.Where(x => x.Task == task)
                .Sum(x => (long) x.Counter)
                .ToString()
                .PadLeft(5, ' ');
            var sum = _threads.Values.Where(x => x.Task == task).Sum(x => x.ResultSum);
            var ssum = sum == 0 ? "" : $"[{sum.ToString("n0")}] - ";
            var meanRuntime = TimeSpan.FromMilliseconds(
                _threads.Values
                    .Where(x => x.Task == task)
