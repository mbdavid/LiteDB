...
internal class BsonExpressionParser
{
    ...
    /// </summary>
    private static readonly Dictionary<string, Tuple<string, MethodInfo, BsonExpressionType>> _operators =
        new Dictionary<string, Tuple<string, MethodInfo, BsonExpressionType>>
        {
            // arithmetic
    ...
    /// </summary>
    public static BsonExpression ParseFullExpression(
        Tokenizer tokenizer,
            ...
            else
            {
                ...
                else
                {
                    ...
                    result = new BsonExpression
                    {
                        ...
                        IsScalar = true,
                        Fields = new HashSet<string>(StringComparer.OrdinalIgnoreCase).AddRange(left.Fields)
                            .AddRange(right.Fields),
    ...
    ///     Start parse string into linq expression. Read path, function or base type bson values (int, double, bool, string)
    /// </summary>
    public static BsonExpression ParseSingleExpression(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    ...
    /// </summary>
    public static BsonExpression ParseSelectDocumentBuilder(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters)
    {
        ...
        return new BsonExpression
        {
            ...
            IsScalar = true,
            Fields = new HashSet<string>(StringComparer.OrdinalIgnoreCase).AddRange(
                fields.SelectMany(x => x.Value.Fields)),
    ...
    /// </summary>
    public static BsonExpression ParseUpdateDocumentBuilder(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters)
    {
    ...
    /// </summary>
    private static BsonExpression TryParseDocument(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    ...
    /// </summary>
    private static BsonExpression TryParseSource(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    {
        ...
        if (tokenizer.LookAhead(false).Type == TokenType.Period)
        {
            ...
            var pathExpr = BsonExpression.ParseAndCompile(
                tokenizer,
                BsonExpressionParserMode.Single,
                parameters,
            ...
            return new BsonExpression
            {
                ...
                Expression = Expression.Call(
                    BsonExpression.GetFunction("MAP"),
                    context.Root,
                    context.Collation,
                    context.Parameters,
                    sourceExpr.Expression,
                    Expression.Constant(pathExpr)),
    ...
    /// </summary>
    private static BsonExpression TryParseArray(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    ...
    /// </summary>
    private static BsonExpression TryParseParameter(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    ...
    /// </summary>
    private static BsonExpression TryParseMethodCall(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    {
        ...
        else
        {
            while (!tokenizer.CheckEOF())
            {
                ...
                // update isImmutable only when came false
                if (parameter.IsImmutable == false)
                    isImmutable = false;
        ...
        if (method == null)
            throw LiteException.UnexpectedToken(
                $"Method '{token.Value.ToUpperInvariant()}' does not exist or contains invalid parameters",
                token);
        ...
        foreach (var item in method.GetParameters()
            .Where(x => x.ParameterType != typeof(Collation))
            .Zip(pars, (parameter, expr) => new { parameter, expr }))
        {
    ...
    /// </summary>
    private static BsonExpression TryParsePath(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    {
        // test $ or @ or WORD
        if (tokenizer.Current.Type != TokenType.At &&
            tokenizer.Current.Type != TokenType.Dollar &&
            tokenizer.Current.Type != TokenType.Word)
            return null;
        ...
        var expr = Expression.Call(
            _memberPathMethod,
            defaultScope == TokenType.Dollar ? context.Root : context.Current,
            name) as Expression;
        ...
        if (isScalar == false && tokenizer.LookAhead(false).Type == TokenType.Period)
        {
            ...
            var mapExpr = BsonExpression.ParseAndCompile(
                tokenizer,
                BsonExpressionParserMode.Single,
                parameters,
    ...
    /// </summary>
    private static Expression ParsePath(
        Tokenizer tokenizer,
        Expression expr,
        ExpressionContext context,
        BsonDocument parameters,
        ...
        StringBuilder src)
    {
        ...
        if (ahead.Type == TokenType.OpenBracket) // array 
        {
            ...
            {
                // inner expression
                inner = BsonExpression.ParseAndCompile(
                    tokenizer,
    ...
    /// </summary>
    private static BsonExpression TryParseFunction(
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    {
        ...
        {
            case "MAP":
                return ParseFunction(token, BsonExpressionType.Map, tokenizer, context, parameters, scope);
            case "FILTER":
                return ParseFunction(token, BsonExpressionType.Filter, tokenizer, context, parameters, scope);
            case "SORT":
                return ParseFunction(token, BsonExpressionType.Sort, tokenizer, context, parameters, scope);
        }
    ...
    ///     MAP(items[*] => @.Name)
    /// </summary>
    private static BsonExpression ParseFunction(
        string functionName,
        BsonExpressionType type,
        Tokenizer tokenizer,
        ExpressionContext context,
        BsonDocument parameters,
        DocumentScope scope)
    {
        ...
        if (tokenizer.LookAhead().Type == TokenType.Equals)
        {
            ...
            var right = BsonExpression.ParseAndCompile(
                tokenizer,
                BsonExpressionParserMode.Full,
                parameters,
    ...
    private static BsonExpression NewArray(BsonExpression item0, BsonExpression item1)
    {
        ...
        return new BsonExpression
        {
            ...
            IsScalar = true,
            Fields =
                new HashSet<string>(StringComparer.OrdinalIgnoreCase).AddRange(item0.Fields).AddRange(item1.Fields),
    ...
    /// </summary>
    internal static BsonExpression CreateLogicExpression(
        BsonExpressionType type,
        BsonExpression left,
    ...
    /// </summary>
    internal static BsonExpression CreateConditionalExpression(
        BsonExpression test,
        BsonExpression ifTrue,
        BsonExpression ifFalse)
    {
        // convert BsonValue into Boolean
        ...
        var result = new BsonExpression
        {
            ...
            Fields = new HashSet<string>(StringComparer.OrdinalIgnoreCase).AddRange(test.Fields)
                .AddRange(ifTrue.Fields)
                .AddRange(ifFalse.Fields),
            Expression = expr,
