...
///     indexes, updates). See https://github.com/mbdavid/LiteDB/wiki/Expressions
/// </summary>
public sealed class BsonExpression
{
    ...
    /// <summary>
    ///     Execute scalar expression over single document and return a single value (or BsonNull when empty). Throws exception
    ...
    /// <summary>
    ///     Execute scalar expression over multiple documents and return a single value (or BsonNull when empty). Throws
    ///     exception if expression are not scalar expression
    /// </summary>
        ...
        DocumentScope scope)
    {
        ...
        var expr =
            mode == BsonExpressionParserMode.Full
    ...
    private static readonly Dictionary<string, MethodInfo> _methods =
        typeof(BsonExpressionMethods).GetMethods(BindingFlags.Public | BindingFlags.Static)
            .ToDictionary(
                m => m.Name.ToUpperInvariant() +
                    "~" +
                    m.GetParameters().Where(p => p.ParameterType != typeof(Collation)).Count());
