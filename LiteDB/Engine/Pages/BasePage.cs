...
internal class BasePage
{
    ...
    /// <summary>
    ///     Get how many free bytes (including fragmented bytes) are in this page (content space) - Will return 0 bytes if page
    ///     are full (or with max 255 items)
    /// </summary>
    ...
    /// <summary>
    ///     Set in all datafile pages the page id about data/index collection. Useful if want re-build database without any
    ///     index [4 bytes]
    /// </summary>
    ...
    private BufferSlice InternalInsert(ushort bytesLength, ref byte index)
    {
        ...
        ENSURE(
            continuousBlocks == PAGE_SIZE - NextFreePosition - FooterSize - (isNewInsert ? SLOT_SIZE : 0),
    ...
    /// <summary>
    ///     Update segment bytes with new data. Current page must have bytes enougth for this new size. Index will not be
    ///     changed
    ...
    ///     to create a single continuous content area (just after header area). No index segment will be changed (only
    ///     positions)
    /// </summary>
    public void Defrag()
    {
