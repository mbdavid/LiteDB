...
internal class DiskWriterQueue : IDisposable
{
    ...
    private Exception _exception; // store last exception in async running task
    ...
    /// <summary>
    ///     Add page into writer queue and will be saved in disk by another thread. If page.Position = MaxValue, store at end
    ///     of file (will get final Position)
    ...
    /// <summary>
    ///     Wait until all queue be executed and no more pending pages are waiting for write - be sure you do a full lock
    ///     database before call this
    /// </summary>
    public void Wait()
    {
        _queueIsEmpty.Wait();
