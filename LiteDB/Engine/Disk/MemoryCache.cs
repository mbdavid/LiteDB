...
internal class MemoryCache : IDisposable
{
    ...
    public PageBuffer MoveToReadable(PageBuffer page)
    {
        ...
        var readable = _readable.AddOrUpdate(
            key,
            page,
            (newKey, current) =>
            {
    ...
    /// </summary>
    public int ExtendPages => Enumerable.Range(0, _extends)
        .Select(x => _segmentSizes[Math.Min(_segmentSizes.Length - 1, x)])
        .Sum();
    ...
    public int WritablePages => ExtendPages - // total memory
        _free.Count -
        _readable.Count; // allocated pages
