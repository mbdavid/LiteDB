...
internal class TransactionService : IDisposable
{
    ...
    /// <summary>
    ///     Finalizer: Will be called once a thread is closed. The TransactionMonitor._slot releases the used
    ///     TransactionService.
    /// </summary>
    ~TransactionService()
    {
    ...
    public Snapshot CreateSnapshot(LockMode mode, string collection, bool addIfNotExists)
    {
        ...
        if (_snapshots.TryGetValue(collection, out var snapshot))
        {
            ...
            if ((mode == LockMode.Write && snapshot.Mode == LockMode.Read) ||
                (addIfNotExists && snapshot.CollectionPage == null))
            {
    ...
    public void Rollback()
    {
        ENSURE(
            ...
            "transaction must be active to rollback (current state: {0})",
            _state);

        LOG(
            $"rollback transaction ({_transPages.TransactionSize} pages with {_transPages.NewPages.Count} returns)",
            "TRANSACTION");
    ...
    /// <summary>
    ///     Return added pages when occurs an rollback transaction (run this only in rollback). Create new transactionID and
    ///     add into
        ...
        lock (_header)
        {
            ...
            IEnumerable<PageBuffer> source()
            {
                ...
                for (var i = 0; i < _transPages.NewPages.Count; i++)
                {
                    ...
                    var next = i < _transPages.NewPages.Count - 1
                        ? _transPages.NewPages[i + 1]
                        : _header.FreeEmptyPageList;
