...
internal class WalIndexService
{
    ...
    private readonly Dictionary<uint, List<KeyValuePair<int, long>>> _index =
        new Dictionary<uint, List<KeyValuePair<int, long>>>();
    ...
    /// <summary>
    ///     Checks if a Page/Version are in WAL-index memory. Consider version that are below parameter. Returns PagePosition
    ///     of this page inside WAL-file or Empty if page doesn't found.
    /// </summary>
    ...
    /// <summary>
    ///     Do checkpoint operation to copy log pages into data file. Return how many transactions was commited inside data
    ///     file
    ...
    public int Checkpoint()
    {
        ...
        if (_disk.GetVirtualLength(FileOrigin.Log) == 0 || _confirmTransactions.Count == 0)
            return 0;
    ...
    public int TryCheckpoint()
    {
        ...
        if (_locker.TryEnterExclusive(out var mustExit) == false)
            return 0;
