...
internal class IndexService
{
    ...
    public CollectionIndex CreateIndex(string name, string expr, bool unique)
    {
        ...
        var head = indexPage.InsertIndexNode(
            index.Slot,
            MAX_LEVEL_LENGTH,
            BsonValue.MinValue,
            PageAddress.Empty,
            bytesLength);
        var tail = indexPage.InsertIndexNode(
            index.Slot,
            MAX_LEVEL_LENGTH,
            BsonValue.MaxValue,
            PageAddress.Empty,
    ...
    ///     If not found but sibling = true and key are not found, returns next value index node (if order = Asc) or prev node
    ///     (if order = Desc)
    /// </summary>
