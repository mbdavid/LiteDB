...
public class LiteQueryable<T> : ILiteQueryable<T>
{
    ...
    public ILiteQueryable<T> OrderByDescending<K>(Expression<Func<T, K>> keySelector) =>
        OrderBy(keySelector, Query.Descending);
    ...
    /// <summary>
    ///     Filter documents after group by pipe according to predicate expression (requires GroupBy and support only one
    ///     Having)
    /// </summary>
    ...
    /// <summary>
    ///     Execute query locking collection in write mode. This is avoid any other thread change results after read document
    ///     and before transaction ends
    /// </summary>
