...
public partial class BsonMapper
{
    ...
    private readonly ConcurrentDictionary<Type, Func<BsonValue, object>> _customDeserializer =
    ...
    protected virtual EntityMapper BuildAddEntityMapper(Type type)
    {
        ...
        _entities[type] =
            mapper; //direct add into entities, to solove the DBRef [ GetEntityMapper > BuildAddEntityMapper > RegisterDbRef > RegisterDbRefItem > GetEntityMapper ] Loop call recursion,we stoped at here and GetEntityMapper's _entities.TryGetValue
    ...
    {
        return Reflection.SelectMember(
            members,
        ...
        if (IncludeFields)
        {
            members.AddRange(
                type.GetFields(flags)
                    .Where(x => !x.Name.EndsWith("k__BackingField") && x.IsStatic == false)
                    .Select(x => x as MemberInfo));
        }
        ...
        foreach (ConstructorInfo ctor in type.GetConstructors())
        {
            ...
            CreateObject toAdd = value =>
                Activator.CreateInstance(
                    ...
                    paramMap.Select(
                            x =>
                                Deserialize(x.Value, value[x.Key]))
                        .ToArray());
    ...
    /// </summary>
    internal static void RegisterDbRef(
        BsonMapper mapper,
        MemberMapper member,
        ITypeNameBinder typeNameBinder,
        string collection)
    {
        member.IsDbRef = true;
    ...
    ///     only
    /// </summary>
    private static void RegisterDbRefItem(
        BsonMapper mapper,
        MemberMapper member,
        ITypeNameBinder typeNameBinder,
        string collection)
    ...
    ///     $ref only
    /// </summary>
    private static void RegisterDbRefList(
        BsonMapper mapper,
        MemberMapper member,
        ITypeNameBinder typeNameBinder,
        string collection)
