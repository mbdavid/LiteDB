...
namespace LiteDB.Demo
{
    public class TestBasePage
    {
        ...
        public static void Run(Stopwatch sw)
        {
            ...
            var pageBuffer = new PageBuffer(buffer, 0, 0) { ShareCounter = -1 };
        ...
        static void InsertDoc(BasePage2 page, BsonDocument doc, int id)
        {
            ...
            var index = page.GetFreeIndex();
            var segment = page.InsertSegment(index, len);

            using (var writer = new BufferWriter(segment))
            {
        ...
        static BsonDocument ReadDoc(BasePage2 page, byte index)
        {
            ...
            using (var reader = new BufferReader(segment))
            {
    ...
    internal class BasePage2 : BasePage
    {
        ...
        public BufferSlice GetSegment(byte index) => base.Get(index);
        public BufferSlice InsertSegment(byte index, int bytesLenth) => base.Insert(index, bytesLenth);
        public BufferSlice UpdateSegment(byte index, int bytesLenth) => base.Update(index, bytesLenth);
        public void DeleteSegment(byte index) => base.Delete(index);
